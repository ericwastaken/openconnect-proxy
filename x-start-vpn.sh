...
profile_proxy_port() {
  profile_env_value "$1" "PROXY_PORT"
}

profile_auth_mode() {
  local auth_mode

  auth_mode="$(profile_env_value "$1" "AUTH_MODE")"
  printf '%s' "${auth_mode:-password}" | tr '[:upper:]' '[:lower:]'
}

profile_saml_auth_port() {
  local saml_auth_port

  saml_auth_port="$(profile_env_value "$1" "SAML_AUTH_PORT")"
  echo "${saml_auth_port:-8080}"
}

profile_image() {
  local image

  image="$(profile_env_value "$1" "IMAGE")"
  echo "${image:-ericwastakenondocker/openconnect-proxy:latest}"
}
...
profile_env_value() {
  ...
  sed -n "s/^${key}=//p" "$profile" | tail -n 1 | sed 's/^["'\'']//; s/["'\'']$//'
}

compose_command_for_profile() {
  local profile="$1"
  ...
  if [ "$auth_mode" = "saml" ]; then
    echo "docker compose -f docker-compose.yml -f docker-compose.saml.yml --env-file \"${profile#./}\" -p \"$project_name\" up -d --force-recreate"
  else
    echo "docker compose --env-file \"${profile#./}\" -p \"$project_name\" up -d --force-recreate"
  fi
}
...
for profile in "${profiles[@]}"; do
  ...
  auth_mode="$(profile_auth_mode "$profile")"
  saml_auth_port="$(profile_saml_auth_port "$profile")"
  ...
  if [ -n "$containers" ]; then
    ...
    elif [ "$auth_mode" = "saml" ] && [ -n "$saml_auth_port" ]; then
      port_containers="$(running_containers_for_port "$saml_auth_port")"
      if [ -n "$port_containers" ]; then
        duplicate_port_profiles+=("$profile")
        duplicate_port_details+=("SAML auth port $saml_auth_port: $port_containers")
      else
...
echo
echo "Future command:"
compose_command_for_profile "$profile" "$project_name"
echo
...
if [ "$(profile_auth_mode "$profile")" = "saml" ]; then
  docker compose -f docker-compose.yml -f docker-compose.saml.yml --env-file "$profile" -p "$project_name" up -d --force-recreate
else
  docker compose --env-file "$profile" -p "$project_name" up -d --force-recreate
fi
